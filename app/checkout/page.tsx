...
export default function CheckoutPage() {
  ...
  const subtotal = useStore(useCartStore, (state) => state.getCartSubtotal) || 0;
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data } = await axiosInstance.post("/api/payments/create-order", orderPayload);

      // 3. Configure Razorpay Gateway Options
      const options = {
        key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "rzp_test_Snk2n1Y872bwmJ",
        ...
        handler: async function (response: any) {
          try {
            ...
            await axiosInstance.post("/api/payments/verify-payment", verificationPayload);
            
            toast.success("Payment Captured Successfully!");
            clearCart();
            router.push("/"); 
          } catch (verificationError) {
            toast.error("Cryptographic signature verification exception caught.");
          }
        },
        prefill: {
      ...
      toast.error(err.response?.data?.message || "Order initialization failed.");
    } finally {
      setProcessing(false);
    }
  };
    ...
    <main className="min-h-screen bg-neutral-950 text-white pt-24 pb-16 px-4 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
        ...
        <form onSubmit={handleCheckoutSubmit} className="lg:col-span-7 space-y-8">
          <div>
            <span className="text-[10px] font-black tracking-widest text-neutral-500 uppercase">Secure Gateway Node</span>
            <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tighter mt-1">Shipping & Checkout</h1>
          </div>

          <div className="space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-widest text-neutral-400 flex items-center gap-2">
              <Truck className="h-4 w-4 text-white" /> Delivery Logistics Vectors
            </h3>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <input
                type="text" required placeholder="First Name" name="firstName"
                value={shippingForm.firstName} onChange={handleInputChange}
              ...
              />
              <input
                type="text" required placeholder="Last Name" name="lastName"
                value={shippingForm.lastName} onChange={handleInputChange}
                className="w-full bg-neutral-900 border border-neutral-850 rounded-none px-4 py-4 text-sm text-white placeholder-neutral-600 focus:outline-none focus:border-neutral-500 transition-colors"
              />
            </div>

            <input
              type="text" required placeholder="Street Address" name="address"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <input
                type="text" required placeholder="City" name="city"
                value={shippingForm.city} onChange={handleInputChange}
                className="w-full bg-neutral-900 border border-neutral-850 rounded-none px-4 py-4 text-sm text-white placeholder-neutral-600 focus:outline-none focus:border-neutral-500 transition-colors"
              />
              <input
                type="text" required placeholder="State" name="state"
                value={shippingForm.state} onChange={handleInputChange}
                className="w-full bg-neutral-900 border border-neutral-850 rounded-none px-4 py-4 text-sm text-white placeholder-neutral-600 focus:outline-none focus:border-neutral-500 transition-colors"
              />
              <input
                type="text" required placeholder="Postal Code" name="postalCode"
            ...
            <input
              type="tel" required placeholder="Contact Phone Number" name="phone"
        ...
        <div className="lg:col-span-5 bg-neutral-950 border border-neutral-900 p-6 lg:sticky lg:top-28">
            ...
            {cart.map((item, idx) => (
              <div key={idx} className="flex gap-4 py-4 first:pt-0 last:pb-0">
                <div className="relative w-12 aspect-[3/4] bg-neutral-900 shrink-0 border border-neutral-900">
                  <Image src={item.product.images?.[0] || "/placeholder.jpg"} alt={item.product.name} fill className="object-cover" sizes="48px" />
                </div>
                <div className="flex-1 min-w-0 text-xs">
                  <h4 className="font-black uppercase truncate text-neutral-200">{item.product.name}</h4>
                  <p className="text-neutral-500 mt-0.5">Size: {item.selectedSize} // Qty: {item.quantity}</p>
                </div>
          ...
          <div className="border-t border-neutral-900 mt-6 pt-6 space-y-3 text-xs">
            ...
            <div className="border-t border-neutral-900 pt-4 flex justify-between items-baseline">
              <span className="text-xs font-black uppercase tracking-widest text-neutral-300">Total Valuation Matrix</span>
          ...
          <div className="mt-6 p-4 bg-neutral-900/40 border border-neutral-900/60 flex gap-3 items-center">
            ...
            <p className="text-[10px] text-neutral-500 font-medium leading-normal">
              Transaction channel fully encrypted via secure socket layer layers. Compliance parameters verified.
            </p>
          </div>
        </div>
